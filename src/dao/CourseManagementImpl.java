...
public class CourseManagementImpl implements CourseManagementDao {
	...
	public Student getStudentDetails(int studentId) {
		...
		Transaction tx = hs.beginTransaction();
		try {
			s = hs.get(Student.class, studentId);
			//access size of collection
			System.out.println(s.getPhoneNumbers().size());
			System.out.println(s.getVehicles().size());
			tx.commit();
		} catch (HibernateException e) {
			if (tx != null)
				tx.rollback();
	...
	public String assignStudentDetails(Student s) {
		//s --detached
		String mesg="Student details added";
		Session hs = getSf().getCurrentSession();
		// tx
		Transaction tx = hs.beginTransaction();
		try {
			hs.update(s);//s ---PERSISTENT
			tx.commit();
		} catch (HibernateException e) {
			if (tx != null)
				tx.rollback();
